...
const DeleteCategorySheet = ({ bottomSheetRef, categories, onDelete, snapPoints }) => {

    const handleDelete = (id, name) => {
        Alert.alert(
            "Confirmar Eliminación",
            `¿Estás seguro de que quieres eliminar la categoría "${name}"? Se perderán sus límites y gastos asociados.`,
            [
    ...
    return (
        ...
        >
            <BottomSheetScrollView contentContainerStyle={sheetStyles.bView}>
                <Text style={sheetStyles.titulo}>Eliminar Categorías</Text>
                
                {categories.length === 0 ? (
                    <Text style={sheetStyles.emptyListText}>No hay categorías para eliminar.</Text>
                ) : (
                    categories.map((item) => (
                        <View key={item.id} style={deleteSheetStyles.deleteItem}>
                            <Text style={deleteSheetStyles.deleteItemText}>{item.name}</Text>
                            <TouchableOpacity 
                                style={deleteSheetStyles.deleteButton}
                                onPress={() => handleDelete(item.id, item.name)}
                            >
                                <Text style={deleteSheetStyles.deleteButtonText}>X</Text>
                            </TouchableOpacity>
                        </View>
                    ))
                )}
                <TouchableOpacity
                    style={[sheetStyles.THBtn, deleteSheetStyles.closeButton]}
                    onPress={() => bottomSheetRef.current?.close()}
                >
                    <Text style={sheetStyles.BtnTxt}>Cerrar</Text>
                </TouchableOpacity>
            </BottomSheetScrollView>
        </BottomSheet>
    );
...
export default function PresupuestosScreen() {
    ...
    const bottomSheetDeleteRef = useRef(null); 

    const snapPoints = useMemo(() => [0.1, "50%"], []);
    const snapPointsForm = useMemo(() => [0.1, "60%"], []);
    const snapPointsDelete = useMemo(() => [0.1, "70%"], []); 
    ...
    const modalOptions = useCallback((item) => {
        Alert.alert(
    ...
    const handleSaveCategory = useCallback((newCategoryData) => {
        if (isEditing) {
            setCategories(prev => 
                prev.map(cat => 
                    cat.id === newCategoryData.id 
                    ? { ...cat, name: newCategoryData.name, limit: newCategoryData.limit } 
                    : cat
                )
            );
        } else {
            ...
            <View style={styles.container}>
                ...
                <ScrollView contentContainerStyle={styles.scrollContent}>
                    ...
                    <View style={styles.budgetCard}>
                        ...
                        <TextInput
                            ...
                            onChangeText={text => setPresupuestoMensual(text.replace(/[^0-9.]/g, ''))} 
                ...
                >
                    <BottomSheetScrollView contentContainerStyle={sheetStyles.bView}>
                        ...
                        <TouchableHighlight 
                            style={[sheetStyles.THBtn, deleteSheetStyles.manageButton]} 
                            underlayColor={'#FEE2E2'}
                            onPress={handleOpenDeleteSheet}
                        >
                            <Text style={deleteSheetStyles.manageButtonText}>Gestionar/Eliminar Categorías</Text>
                        </TouchableHighlight>
                ...
                <DeleteCategorySheet
                    bottomSheetRef={bottomSheetDeleteRef}
